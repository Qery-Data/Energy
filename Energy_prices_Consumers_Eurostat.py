...
#Natural gas price household consumers composition Europe
...
#Natural gas price household consumers PPS Europe
...
#Natural gas prices non-household consumers 

#Natural gas prices over time non-household consumers EU27
dataset = pyjstat.Dataset.read('https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/nrg_pc_203?lang=en&nrg_cons=GJ10000-99999&unit=KWH&tax=X_TAX&tax=X_VAT&tax=I_TAX&currency=EUR&geo=EU27_2020')
df = dataset.write('dataframe')
df.replace(rename_dict, inplace=True)
df_new = df.pivot(index='Taxes', columns='Time', values='value')
df_new.columns = [col.replace('-S1', ' June') if '-S1' in col else col.replace('-S2', ' December') for col in df_new.columns]
...
df_new = df.pivot(index='Geopolitical entity (reporting)', columns='Time', values='value')
df_new.dropna(how='all', inplace=True)
df_new.to_csv('data_Eurostat_Energy_Prices/Natural_Gas_Prices_Non_Household_Consumers_Latest_Europe.csv', index=True)

#Natural gas price non-household consumers comparison by time
dataset = pyjstat.Dataset.read('https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/nrg_pc_203?lang=en&lastTimePeriod=4&nrg_cons=GJ10000-99999&unit=KWH&tax=X_VAT&currency=NAC')
df = dataset.write('dataframe')
df.replace(rename_dict, inplace=True)
df_new = df.pivot(index='Geopolitical entity (reporting)', columns='Time', values='value')
df_new["Pct_change"] = ((df_new.iloc[:, -1] - df_new.iloc[:, 1]) / df_new.iloc[:, 1]) * 100
df_new.dropna(inplace=True)
df_new.to_csv('data_Eurostat_Energy_Prices/Natural_Gas_Prices_Non_Household_Consumers_Change_Europe.csv', index=True)

#Natural gas price non-household by consumer group 
dataset = pyjstat.Dataset.read('https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/nrg_pc_203?lang=en&lastTimePeriod=1&unit=KWH&tax=X_VAT&currency=EUR')
df = dataset.write('dataframe')
df.replace(rename_dict, inplace=True)
df_new = df.pivot(index='Geopolitical entity (reporting)', columns='Energy consumption', values='value')
df_new.dropna(how='all', inplace=True)
df_new.to_csv('data_Eurostat_Energy_Prices/Natural_Gas_Prices_Non_Household_Consumers_Groups_Europe.csv', index=True)

#Natural gas price non-household consumers composition Europe
dataset = pyjstat.Dataset.read('https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/nrg_pc_203?lang=en&lastTimePeriod=1&nrg_cons=GJ10000-99999&unit=KWH&currency=EUR')
df = dataset.write('dataframe')
df.replace(rename_dict, inplace=True)
df_new = df.pivot(index='Geopolitical entity (reporting)', columns='Taxes', values='value')
df_new["Share of taxes and levies (%)"] = ((df_new["Excluding VAT and other recoverable taxes and levies"] - df_new["Excluding taxes and levies"]) /(df_new["Excluding VAT and other recoverable taxes and levies"]) * 100)
df_new.dropna(how='all', inplace=True)
df_new.to_csv('data_Eurostat_Energy_Prices/Natural_Gas_Prices_Non_Household_Consumers_Composition_Europe.csv', index=True)

#Natural gas price non-household consumers PPS Europe
dataset = pyjstat.Dataset.read('https://ec.europa.eu/eurostat/api/dissemination/statistics/1.0/data/nrg_pc_203?lang=en&lastTimePeriod=1&nrg_cons=GJ10000-99999&unit=KWH&tax=X_VAT&currency=EUR&currency=PPS')
df = dataset.write('dataframe')
df.replace(rename_dict, inplace=True)
df_new = df.pivot(index='Geopolitical entity (reporting)', columns='Currency', values='value')
df_new = df_new.rename(columns={"Euro": "EUR", "Purchasing Power Standard": "PPS"})
df_new.dropna(inplace=True)
df_new.to_csv('data_Eurostat_Energy_Prices/Natural_Gas_Prices_Non_Household_Consumers_PPS_Europe.csv', index=True)